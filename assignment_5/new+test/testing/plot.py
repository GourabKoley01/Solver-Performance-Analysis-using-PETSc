...
for (ksp, pc), grp in df.groupby(["KSP", "PC"]):
    plt.plot(grp["Nodes"], grp["Time"], marker="*", markersize=8, label=f"{ksp}+{pc}")
    # Annotate grid labels like "5x5", "10x10", etc.
    for _, row in grp.iterrows():
        label = f"{int(row['Nx'])}x{int(row['Ny'])}"
        #plt.text(row["Nodes"], row["Time"], label, fontsize=8, ha='left', va='bottom')

plt.xlabel("Number of Nodes (Nx x Ny)")
...
plt.legend(title="Solver + Preconditioner", bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small')
plt.tight_layout(rect=[0, 0, 0.8, 1])  # Make room for legend
plt.savefig(os.path.join(plot_dir, "time_vs_nodes_labeled.png"), dpi=300)
plt.close()
...
for (ksp, pc), grp in df_conv.groupby(["KSP", "PC"]):
    plt.plot(grp["Nodes"], grp["Iterations"], marker="o", markersize=8, label=f"{ksp}+{pc}")
    # Annotate grid labels
    for _, row in grp.iterrows():
        label = f"{int(row['Nx'])}x{int(row['Ny'])}"
        #plt.text(row["Nodes"], row["Iterations"], label, fontsize=8, ha='left', va='bottom')
...
plt.savefig(os.path.join(plot_dir, "iters_vs_nodes_labeled.png"), dpi=300)
plt.close()
